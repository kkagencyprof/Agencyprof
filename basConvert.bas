...
Public Function cv_BytesFromString(strInput As String) As Variant
...
' E.g. aBytes() = {&HFE, &HDC, &H80} will return "FEDC80"
    ...
    cv_HexFromBytes = String((UBound(aBytes) - LBound(aBytes) + 1) * 2, " ")
    iIndex = 0
...
Public Function testBytesHex()
    Dim aBytes() As Byte
    Dim str As String
    
    aBytes() = cv_BytesFromHex("FEDC80")
    
    str = cv_StringFromHex("FEDC80")
    aBytes() = cv_BytesFromString(str)
